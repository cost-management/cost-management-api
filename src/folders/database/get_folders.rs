...
pub async fn get_folders(user_id: &Uuid) -> Result<Value, Error> {
    ...
    let folders = match sqlx::query_as::<_, Folder>("select folder.id, title, folder_type, amount, currency, skin, color, folder.created_at, customer_id, customer_role, email from folder join customer_folder on customer_folder.folder_id = folder.id join customer c on c.id = customer_folder.customer_id where folder.id in (select folder_id from customer_folder where customer_folder.customer_id = $1);")
        .bind(user_id)
